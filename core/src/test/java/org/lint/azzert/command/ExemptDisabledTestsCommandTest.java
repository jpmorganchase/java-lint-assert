...
public class ExemptDisabledTestsCommandTest {
    ...
    void test() throws Exception {
        Set<TestMethodContext> methods = new ToStringAssertProcessor(null, Pair.with(ExemptDisabledTestsCommandTest.class.getPackage().getName(), false)).process();

        System.out.println(new ToStringStrategy(methods).render());
        methods.forEach(System.out::println);

        final Function<String, Long> assertsInMethod = str -> methods.stream().filter(m-> m.getMethodName().equals(str)).count();
